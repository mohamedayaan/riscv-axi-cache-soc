...
initial begin
    ...
    rst_n=0; cache_req=0; cache_wen=0; cache_addr=0; cache_wdata=0; cache_burst_len=0;
    repeat(5) @(posedge clk); rst_n=1; repeat(2) @(posedge clk);

    $display("===== AXI4 TEST SUITE =====");

    // Test 1: Single beat write/read
    $display("--- Test 1: Single beat write/read ---");
    cache_addr=32'h10; cache_wdata=32'hDEADBEEF; cache_wen=1; cache_req=1; cache_burst_len=8'd0;
    @(posedge cache_ack); @(posedge clk); cache_req=0;
    repeat(2) @(posedge clk);
    cache_addr=32'h10; cache_wen=0; cache_req=1; cache_burst_len=8'd0;
    @(posedge cache_ack); @(posedge clk); cache_req=0;
    ...
    // Test 2: Burst write 4 words
    $display("--- Test 2: Burst write 4 words ---");
    cache_addr=32'h100; cache_wdata=32'hAAAA0000; cache_wen=1; cache_req=1; cache_burst_len=8'd0;
    ...
    // Test 3: Burst read 4 words
    $display("--- Test 3: Burst read 4 words ---");
